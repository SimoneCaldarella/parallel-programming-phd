...
void runJacobi1DCUDA(int tsteps, int n, double* A, double* B, double* A_outputFromGpu, double* B_outputFromGpu) {
  ...
  for (int t = 0; t < tsteps ; ++t) {
    ...
    cudaDeviceEnablePeerAccess(device1, 0); // Enable peer access
    cudaDeviceEnablePeerAccess(device0, 0);

    cudaSetDevice(device0);
    cudaMemcpyPeer(&d_A0[n/2], device0, &d_A1[0], device1, sizeof(double));
    cudaMemcpyPeer(&d_B0[n/2], device0, &d_B1[0], device1, sizeof(double));

    cudaSetDevice(device1);
    cudaMemcpyPeer(&d_A1[0], device1, &d_A0[n/2], device0, sizeof(double));
    cudaMemcpyPeer(&d_B1[0], device1, &d_B0[n/2], device0, sizeof(double));
 

    // ---> update halos directly using cudaMemcpy transfers version 2 <--- //
    // ---> update halos directly using cudaMemcpy transfers version 2 <--- //
    cudaSetDevice(device0);
    cudaMemcpy(&A_outputFromGpu[n/2], &d_A0[n/2], sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(&B_outputFromGpu[n/2], &d_B0[n/2], sizeof(double), cudaMemcpyDeviceToHost);

    cudaSetDevice(device1);
    cudaMemcpy(&d_A1[0], &A_outputFromGpu[n/2], sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(&d_B1[0], &B_outputFromGpu[n/2], sizeof(double), cudaMemcpyHostToDevice);

    cudaSetDevice(device1);
    cudaMemcpy(&A_outputFromGpu[0], &d_A1[0], sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(&B_outputFromGpu[0], &d_B1[0], sizeof(double), cudaMemcpyDeviceToHost);

    cudaSetDevice(device0);
    cudaMemcpy(&d_A0[n/2], &A_outputFromGpu[0], sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(&d_B0[n/2], &B_outputFromGpu[0], sizeof(double), cudaMemcpyHostToDevice);


    // ---> update through host version 3<--- //
    double halo_from_device0, halo_from_device1;

    cudaSetDevice(device0);
    cudaMemcpy(&halo_from_device0, &d_A0[n/2], sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(&halo_from_device0, &d_B0[n/2], sizeof(double), cudaMemcpyDeviceToHost);

    cudaSetDevice(device1);
    cudaMemcpy(&halo_from_device1, &d_A1[0], sizeof(double), cudaMemcpyDeviceToHost);
    cudaMemcpy(&halo_from_device1, &d_B1[0], sizeof(double), cudaMemcpyDeviceToHost);

    cudaSetDevice(device0);
    cudaMemcpy(&d_A0[n/2], &halo_from_device1, sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(&d_B0[n/2], &halo_from_device1, sizeof(double), cudaMemcpyHostToDevice);

    cudaSetDevice(device1);
    cudaMemcpy(&d_A1[0], &halo_from_device0, sizeof(double), cudaMemcpyHostToDevice);
    cudaMemcpy(&d_B1[0], &halo_from_device0, sizeof(double), cudaMemcpyHostToDevice);
